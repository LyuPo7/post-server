...
spec_sqlDAtoText = describe "Testing sqlDAtoText" $ do
    it "Should successfully convert [[SqlValue]] to Text" $ do
      let bob = "Bob" :: FirstName
          userId = 11 :: UserId
          isAdmin = True
...
spec_sqlAtoText = describe "Testing sqlAtoText" $ do
    it "Should successfully convert [SqlValue] to Text" $ do
      let bob = "Bob" :: FirstName
          userId = 11 :: UserId
          isAdmin = True
...
spec_readEitherMa = describe "Testing readEitherMa" $ do
    ...
    it "Should fail with incorrect input" $ do
      let arg = "123A123" :: Text
          argName = "number"
          argE = (Util.readEitherMa arg argName) :: Identity (Either Text Integer)

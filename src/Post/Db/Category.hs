...
getCatId handle title = do
  let logH = DbQSpec.hLogger handle
  catIdSql <- DbQSpec.selectFromWhere handle DbTable.tableCats
              [DbColumn.colIdCat]
...
getCats handle offset = do
  let logH = DbQSpec.hLogger handle
  catsSQL <- DbQSpec.selectFromOrderLimitOffset handle DbTable.tableCats
...
checkIfChildCatIsValid :: Monad m =>
                          DbQSpec.Handle m ->
                          ServerSynonyms.Title ->
                          ServerSynonyms.Title ->
                          m (Either Text ())
...
getCatRecordByCatId :: Monad m =>
                       DbQSpec.Handle m ->
                       ServerSynonyms.CategoryId ->
                       m (Either Text ServerCat.Category)
getCatRecordByCatId handle catId = do
  let logH = DbQSpec.hLogger handle
  catsSql <- DbQSpec.selectFromWhere handle DbTable.tableCats
              [DbColumn.colIdCat, DbColumn.colTitleCat, DbColumn.colSubCatCat]
              [DbColumn.colIdCat]
              [toSql catId]
  case catsSql of
...
getCatPostIdsByCatId :: Monad m =>
                        DbQSpec.Handle m ->
                        ServerSynonyms.CategoryId ->
                        m (Either Text [ServerSynonyms.PostId])
getCatPostIdsByCatId handle catId = do
  let logH = DbQSpec.hLogger handle
  postsIdSql <- DbQSpec.selectFromWhere handle DbTable.tablePostCat
...
getChildCatIdsByCatId :: Monad m =>
                         DbQSpec.Handle m ->
                         ServerSynonyms.CategoryId ->
                         m (Either Text [ServerSynonyms.CategoryId])
getChildCatIdsByCatId handle catId = do
  let logH = DbQSpec.hLogger handle
  childCatIdSql <- DbQSpec.selectFromWhere handle DbTable.tableCats
                    [DbColumn.colIdCat]
...
insertCatWSubRecord handle title subCatId = do
  let logH = DbQSpec.hLogger handle
  _ <- DbQSpec.insertIntoValues handle DbTable.tableCats 
...
updateCatTitleRecord handle catId newTitle = do
  let logH = DbQSpec.hLogger handle
  _ <- DbQSpec.updateSetWhere handle DbTable.tableCats
        [DbColumn.colTitleCat]
        [DbColumn.colIdCat]
        [toSql newTitle]
        [toSql catId]
...
deleteCatRecord :: Monad m =>
                   DbQSpec.Handle m ->
                   ServerSynonyms.CategoryId ->
                   m ()
deleteCatRecord handle catId = do
  let logH = DbQSpec.hLogger handle
  _ <- DbQSpec.deleteWhere handle DbTable.tableCats
        [DbColumn.colIdCat]
        [toSql catId]
...
newCat handle [idCat, title, subId] = do
  ...
  return $ Right $ ServerCat.Category {
...
newCatNull [idCat, title, _] = return ServerCat.Category {
  ServerCat.title = fromSql title,
  ServerCat.id = fromSql idCat,

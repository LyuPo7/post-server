...
getAuthorRecord :: Monad m =>
                   DbQSpec.Handle m ->
                   ServerSynonyms.AuthorId ->
                   m (Either Text ServerAuthor.Author)
getAuthorRecord handle authorId = do
  let logH = DbQSpec.hLogger handle
  authorSql <- DbQSpec.selectFromWhere handle DbTable.tableAuthors
...
getUserIdByAuthorId :: Monad m =>
                       DbQSpec.Handle m ->
                       ServerSynonyms.AuthorId ->
                       m (Either Text ServerSynonyms.UserId)
getUserIdByAuthorId handle authorId = do
  let logH = DbQSpec.hLogger handle
  idUserSql <- DbQSpec.selectFromWhere handle DbTable.tableAuthorUser
...
getAuthorRecords :: Monad m =>
                    DbQSpec.Handle m ->
                    ServerSynonyms.Offset ->
                    m (Either Text [ServerAuthor.Author])
getAuthorRecords handle offset = do
  let logH = DbQSpec.hLogger handle
  authorsSQL <- DbQSpec.selectFromOrderLimitOffset  handle DbTable.tableAuthors
                 [DbColumn.colIdAuthor, DbColumn.colDescAuthor]
                  offset
  case authorsSQL of
    [] -> do
...
getLastAuthorRecord :: Monad m =>
                       DbQSpec.Handle m ->
                       m (Either Text ServerSynonyms.AuthorId)
getLastAuthorRecord handle = do
  let logH = DbQSpec.hLogger handle
  idAuthorSql <- DbQSpec.selectFromOrderLimit handle DbTable.tableAuthors
                  [DbColumn.colIdAuthor] 
                   DbColumn.colIdAuthor 1
  case idAuthorSql of
    [] -> do
...
getPostIdsByAuthorId :: Monad m =>
                        DbQSpec.Handle m ->
                        ServerSynonyms.AuthorId ->
                        m (Either Text [ServerSynonyms.PostId])
getPostIdsByAuthorId handle authorId = do
  let logH = DbQSpec.hLogger handle
  postsIdSql <- DbQSpec.selectFromWhere handle DbTable.tablePostAuthor
...
updateAuthorRecord handle authorId newDescription = do
  let logH = DbQSpec.hLogger handle
  _ <- DbQSpec.updateSetWhere handle DbTable.tableAuthors
        [DbColumn.colDescAuthor]
        [DbColumn.colIdAuthor]
...
deleteAuthorRecord :: Monad m =>
                      DbQSpec.Handle m ->
                      ServerSynonyms.AuthorId ->
                      m ()
deleteAuthorRecord handle authorId = do
  let logH = DbQSpec.hLogger handle
  _ <- DbQSpec.deleteWhere handle DbTable.tableAuthors
        [DbColumn.colIdAuthor]
        [toSql authorId]
...
insertAuthorUserRecord :: Monad m =>
                          DbQSpec.Handle m -> 
                          ServerSynonyms.AuthorId ->
                          ServerSynonyms.UserId ->
                          m ()
insertAuthorUserRecord handle authorId userId = do
  let logH = DbQSpec.hLogger handle
  _ <- DbQSpec.insertIntoValues handle DbTable.tableAuthorUser 
...
deleteAuthorUserRecord :: Monad m =>
                          DbQSpec.Handle m ->
                          ServerSynonyms.UserId ->
                          m ()
deleteAuthorUserRecord handle userId = do
  let logH = DbQSpec.hLogger handle
  _ <- DbQSpec.deleteWhere handle DbTable.tableAuthorUser
        [DbColumn.colIdUserAuthorUser]
        [toSql userId]

...
removePostMainPhotoDep :: Monad m =>
                          DbQSpec.Handle m ->
                          ServerSynonyms.PostId ->
                          m (Either Text ServerSynonyms.PhotoId)
...
getPostRecord :: Monad m =>
                 DbQSpec.Handle m ->
                 ServerSynonyms.PostId ->
                 m (Either Text ServerPost.Post)
getPostRecord handle postId = do
  let logH = DbQSpec.hLogger handle
  postSQL <- DbQSpec.selectFromWhere handle DbTable.tablePosts
              [
                DbColumn.colIdPost,
...
getLastPostRecord :: Monad m =>
                     DbQSpec.Handle m ->
                     m (Either Text ServerSynonyms.PostId)
getLastPostRecord handle = do
  let logH = DbQSpec.hLogger handle
  idPostSql <- DbQSpec.selectFromOrderLimit handle DbTable.tablePosts
                [DbColumn.colIdPost]
                 DbColumn.colIdPost 1
  case idPostSql of
    [] -> do
...
getPostIdByTitle :: Monad m =>
                    DbQSpec.Handle m ->
                    ServerSynonyms.Title ->
                    m (Either Text ServerSynonyms.PostId)
getPostIdByTitle handle title = do
  let logH = DbQSpec.hLogger handle
  postIdSQL <- DbQSpec.selectFromWhere handle DbTable.tablePosts
                [DbColumn.colIdPost]
                [DbColumn.colTitlePost]
                [toSql title]
  case postIdSQL of
...
getPostAuthorIdByPostId :: Monad m =>
                           DbQSpec.Handle m ->
                           ServerSynonyms.PostId ->
                           m (Either Text ServerSynonyms.AuthorId)
getPostAuthorIdByPostId handle postId = do
  let logH = DbQSpec.hLogger handle
  authorIdSql <- DbQSpec.selectFromWhere handle DbTable.tablePostAuthor
...
getPostCategoryIdByPostId :: Monad m =>
                             DbQSpec.Handle m -> 
                             ServerSynonyms.PostId ->
                             m (Either Text ServerSynonyms.CategoryId)
getPostCategoryIdByPostId handle postId = do
  let logH = DbQSpec.hLogger handle
  catIdSql <- DbQSpec.selectFromWhere handle DbTable.tablePostCat
...
getPostTagIdsByPostId :: Monad m =>
                         DbQSpec.Handle m ->
                         ServerSynonyms.PostId ->
                         m (Either Text [ServerSynonyms.TagId])
getPostTagIdsByPostId handle postId = do
  let logH = DbQSpec.hLogger handle
  tagsIdSql <- DbQSpec.selectFromWhere handle DbTable.tablePostTag
...
getPostMainPhotoIdByPostId :: Monad m =>
                              DbQSpec.Handle m ->
                              ServerSynonyms.PostId ->
                              m (Either Text ServerPhoto.Photo)
getPostMainPhotoIdByPostId handle postId = do
  let logH = DbQSpec.hLogger handle
  photoIdSql <- DbQSpec.selectFromWhere handle DbTable.tablePostMainPhoto
...
getPostAddPhotoIdsByPostId :: Monad m =>
                              DbQSpec.Handle m ->
                              ServerSynonyms.PostId ->
                              m (Either Text [ServerPhoto.Photo])
getPostAddPhotoIdsByPostId handle postId = do
  let logH = DbQSpec.hLogger handle
  photoIdSql <- DbQSpec.selectFromWhere handle DbTable.tablePostAddPhoto
...
getPostDraftIdByPostId :: Monad m =>
                          DbQSpec.Handle m ->
                          ServerSynonyms.PostId ->
                          m (Either Text ServerSynonyms.DraftId)
getPostDraftIdByPostId handle postId = do
  let logH = DbQSpec.hLogger handle
  draftIdSql <- DbQSpec.selectFromWhere handle DbTable.tablePostDraft
...
getPostDraftIdsByPostIds :: Monad m =>
                            DbQSpec.Handle m ->
                           [ServerSynonyms.PostId] ->
                            m (Either Text [ServerSynonyms.DraftId])
getPostDraftIdsByPostIds handle postIds = do
  let logH = DbQSpec.hLogger handle
  draftIdsSql <- DbQSpec.selectFromWhereIn handle DbTable.tablePostDraft
                 [DbColumn.colIdDraftPostDraft]
                  DbColumn.colIdPostPostDraft
                  $ map toSql postIds
  case draftIdsSql of
...
getPostCommentRecords :: Monad m =>
                         DbQSpec.Handle m ->
                         ServerSynonyms.PostId ->
                         m (Either Text [ServerComment.Comment])
getPostCommentRecords handle postId = do
  let logH = DbQSpec.hLogger handle
  comsIdSql <- DbQSpec.selectFromWhere handle DbTable.tablePostCom
...
insertPostRecord :: Monad m =>
                    DbQSpec.Handle m ->
                    ServerSynonyms.Title -> Text -> m ()
insertPostRecord handle title text = do
  let logH = DbQSpec.hLogger handle
  time <- DbQSpec.getCurrentTime handle
  let day = utctDay time
  _ <- DbQSpec.insertIntoValues handle DbTable.tablePosts
...
updatePostMainPhotoRecord handle postId photoId = do
  let logH = DbQSpec.hLogger handle
  _ <- DbQSpec.updateSetWhere handle DbTable.tablePostMainPhoto
        [DbColumn.colIdPhotoPostMainPhoto]
        [DbColumn.colIdPostPostMainPhoto]
        [toSql photoId]
        [toSql postId]
...
insertPostMainPhotoRecord :: Monad m =>
                             DbQSpec.Handle m ->
                             ServerSynonyms.PostId ->
                             ServerSynonyms.PhotoId ->
                             m (Either Text ServerSynonyms.PhotoId)
insertPostMainPhotoRecord handle postId photoId = do
  let logH = DbQSpec.hLogger handle
  _ <- DbQSpec.insertIntoValues handle DbTable.tablePostMainPhoto
...
insertPostAddPhotoRecord :: Monad m =>
                            DbQSpec.Handle m ->
                            ServerSynonyms.PostId ->
                            ServerSynonyms.PhotoId ->
                            m (Either Text ServerSynonyms.PhotoId)
insertPostAddPhotoRecord handle postId photoId = do
  let logH = DbQSpec.hLogger handle
  _ <- DbQSpec.insertIntoValues handle DbTable.tablePostAddPhoto
...
insertPostAuthorRecord :: Monad m =>
                          DbQSpec.Handle m ->
                          ServerSynonyms.PostId ->
                          ServerSynonyms.AuthorId -> m ()
insertPostAuthorRecord handle postId authorId = do
  let logH = DbQSpec.hLogger handle
  _ <- DbQSpec.insertIntoValues handle DbTable.tablePostAuthor 
...
insertPostCatRecord :: Monad m =>
                       DbQSpec.Handle m ->
                       ServerSynonyms.PostId -> 
                       ServerSynonyms.CategoryId -> m ()
insertPostCatRecord handle postId catId = do
  let logH = DbQSpec.hLogger handle
  _ <- DbQSpec.insertIntoValues handle DbTable.tablePostCat 
...
insertPostTagRecord handle postId tagId = do
  let logH = DbQSpec.hLogger handle
  _ <- DbQSpec.insertIntoValues handle DbTable.tablePostTag
...
insertPostDraftRecord handle postId draftId = do
  let logH = DbQSpec.hLogger handle
  _ <- DbQSpec.insertIntoValues handle DbTable.tablePostDraft
...
deletePostRecord :: Monad m =>
                    DbQSpec.Handle m ->
                    ServerSynonyms.PostId ->
                    m ()
deletePostRecord handle postId = do
  let logH = DbQSpec.hLogger handle
  _ <- DbQSpec.deleteWhere handle DbTable.tablePosts
        [DbColumn.colIdPost]
        [toSql postId]
